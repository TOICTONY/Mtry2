...
from bot.helper.ext_utils.metadata_utils import change_metadata_title
...
async def format_filename(file_, user_id, dirpath=None, isMirror=False):
    ...
    metadata = config_dict[f'{ctag}_FILENAME_METADATA'] if (val:=user_dict.get(f'{ftag}metadata', '')) == '' else val
