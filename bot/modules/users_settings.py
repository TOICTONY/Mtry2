...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. \n<b>Timeout:</b> 60 sec'],
            ...
            'lmetadata': ['Leech Filename metadata is for manipulating File data name of the Leech Files', 'Send Leech Filename metadata. Documentation Here : <a href="https://t.me/WZML_X/77">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            ...
            'mmetadata': ['Mirror Filename metadata is for manipulating File data name of the Mirror/Cloned Files', 'Send Mirror Filename metadata. \n<b>Timeout:</b> 60 sec'],
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        text = BotTheme('MIRROR', NAME=name, RCLONE=rccmsg, DDL_SERVER=ddl_serv, DM=f"{dailyup} / {dailytlup}", MREMNAME=escape(mremname), MMETADATA=escape(mmetadata), MPREFIX=escape(mprefix),
        ...
        text = BotTheme('LEECH', NAME=name, DL=f"{dailyll} / {dailytlle}",
                ...
                LSUFFIX=escape(lsuffix), LDUMP=ldump, LREMNAME=escape(lremname), LMETADATA=escape(lmetadata))
        ...
        elif key in ['lprefix', 'lremname', 'lmetadata', 'lsuffix', 'lcaption', 'ldump']:
        ...
        elif key in ['mprefix', 'mremname', 'mmetadata', 'msuffix']:
...
async def user_settings(client, message):
    if len(message.command) > 1 and (message.command[1] == '-s' or message.command[1] == '-set'):
        ...
        if set_arg and (reply_to := message.reply_to_message):
            ...
            if set_arg in ['lprefix', 'lsuffix', 'lremname', 'lmetadata', 'lcaption', 'ldump', 'yt_opt'] and reply_to.text:
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['lprefix', 'lsuffix', 'lremname', 'lmetadata', 'lcaption', 'ldump', 'mprefix', 'msuffix', 'mremname', 'mmetadata']:
        handler_dict[user_id] = False
        await query.answer()
        edit_mode = len(data) == 4
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['dlprefix', 'dlsuffix', 'dlremname', 'dlmetadata', 'dlcaption', 'dldump']:
        handler_dict[user_id] = False
        await query.answer()
